            ...
            {stats ? (
              <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                  <StatCard label="Market Value" value={`$${stats.avgCost.toFixed(2)}`} subValue={`Max: $${stats.maxCost.toFixed(2)}`} tooltip="Calculated USD cost based mean average pulls and worst case." />
                  <StatCard label="Cohort Spending" value={`$${stats.totalCohortInvestment.toLocaleString(undefined, {
                    minimumFractionDigits: 0, 
                    maximumFractionDigits: 0})}`} tooltip="Total gross revenue generated by all sampled players combined." />
                  <StatCard label="Featured Guarantee Revenue" value={`$${stats.guaranteeRevenue.toLocaleString(undefined, {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0})}`} subValue={`${stats.guaranteeRevenuePercentage}% of total`} color="text-amber-700" tooltip="Revenue generated by the players who reached the featured guarantee." />
                </div>
